...
for(const [command,args] of [
  ...
  ['pnpm',['--filter','@coati/api','test','gateway','migration-chain','setup-once']],
]){
  const result=spawnSync(command,args,{cwd:root,stdio:'inherit'})
  if(result.status!==0)process.exit(result.status??1)
}
console.log('Gateway contract, migration and bootstrap checks passed. This does not certify legacy parity or production readiness.')
